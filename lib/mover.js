  ...
  for (const group of job.duplicateGroups.values()) {
    ...
    if (group.resolution.type === 'merge') {
      ...
      for (const file of files) {
        // A file already in handledFileIds here belongs to an EXCLUDED themed folder
        // (pre-seeded above). It must stay physically inside that untouched folder,
        // so it must NOT be pulled out or deleted by duplicate resolution.
      ...
      let idx = 1;
      for (const file of files) {
        if (handledFileIds.has(file.id)) continue; // excluded themed-folder stray - leave it in place
        handledFileIds.add(file.id);
        if (file.excluded) {
          report.skippedExcluded += 1;
          continue;
