...
async function buildPlan(job) {
  ...
  // 2) Detected projects: delete internal junk, then move the folder intact.
  const projectDestDir = path.join(job.destination, PROJECT_CATEGORY);
  for (const project of job.projects || []) {
    ...
    const to = await reserveDestPath(projectDestDir, project.name, reserved);
    push({ kind: 'moveProject', from: project.absPath, to, projectType: project.type });
  }
...
function recordSuccess(report, op) {
  switch (op.kind) {
    ...
    case 'moveProject':
      ...
      report.moved.push({ from: op.from, to: op.to, category: PROJECT_CATEGORY });
      break;
    case 'moveThemed':
