...
async function buildPlan(job) {
  ...
  // 2) Detected projects/applications: delete internal junk, pull out any genuine STRAY
  //    files first (so the folder that moves is clean), then move the folder intact into
  //    its destination category (source projects -> coded_programs, applications -> applications).
  for (const project of job.projects || []) {
    if (project.excluded) {
      // Left untouched: its strays must stay inside it, so shield them from later passes.
      for (const id of project.strayFileIds || []) { handled.add(id); skippedExcluded += 1; }
      skippedExcluded += 1;
      continue;
    }
    for (const junk of project.junkDirsFound || []) push({ kind: 'deleteDir', from: junk.path });
    for (const id of project.strayFileIds || []) {
      const stray = job.files.get(id);
      if (!stray || handled.has(id)) continue;
      handled.add(id);
      if (stray.excluded) { skippedExcluded += 1; continue; }
      const strayTo = await reserveDestPath(destDirFor(stray), stray.name, reserved);
      push({ kind: 'moveFile', from: stray.absPath, to: strayTo, category: stray.category });
    }
