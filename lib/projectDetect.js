...
function detectApplicationFolder(entries, folderName = '') {
  ...
  for (const e of entries) {
    if (e.isDirectory()) { hasSubdir = true; continue; }
    if (!e.isFile()) continue;
    const lower = e.name.toLowerCase();
    if (APP_MARKER_FILES.has(lower)) markerHit = e.name;
  ...
  if (libCount >= 3 || markerHit) {
    return { type: 'Application', markerFile: markerHit || `${libCount} libraries`, strength: 'strong' };
  }
  if (libCount >= 1) {
    return { type: 'Possible application', markerFile: `${libCount} librar${libCount === 1 ? 'y' : 'ies'}`, strength: 'weak' };
  }
  if (folderName && APP_NAME_HINTS.test(folderName) && (hasSubdir || entries.length >= 4)) {
