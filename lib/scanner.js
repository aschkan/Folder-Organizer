...
const { detectProjectMarker, PROJECT_JUNK_DIRS } = require('./projectDetect');
...
const ALWAYS_SKIP_DIR_NAMES = new Set(['.git', '$RECYCLE.BIN', 'System Volume Information']);
...
async function runScan(job) {
  ...
  try {
    ...
    if (stillUnknown.length > 0 && job.useLLM) {
      ...
      const uniqueExts = [...new Set(withExt.map((f) => f.ext))];
      const batchResult = await classifyBatchWithLLM({ extensions: uniqueExts, url: job.llmUrl, model: job.llmModel });
