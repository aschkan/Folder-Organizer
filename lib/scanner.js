 ...
 */
async function resolveFolderStrays(job, opts) {
  ...
  // Apps/programs: only when the AI is on, and conservatively (program resources are never
  // pulled). Bounded per folder; skipped for very large ones (their files all belong).
  if (useAI) {
    for (const project of job.projects) {
      const files = await listFilesInFolder(project.absPath, opts, 400);
      if (files.length === 0 || files.length >= 400) continue;
      logJob(job, `Checking "${project.name}" for files that don't belong to it…`);
      ...
      const set = new Set(rel);
      const strays = files.filter((f) => set.has(f.relPath));
      project.strayFileIds = await materializeStrays(job, strays, project.sourceRoot, 'projectId', project.id);
      if (project.strayFileIds.length > 0) logJob(job, `Pulled ${project.strayFileIds.length} stray file(s) out of "${project.name}".`);
    }
  }
}
...
async function runScan(job) {
  ...
  try {
    ...
    await resolveFolderStrays(job, opts);
